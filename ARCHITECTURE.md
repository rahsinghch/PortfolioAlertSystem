...
Sample picker ──────▶│   as_of, holdings[])      │
              ...
              api/app.py (JSON)        src/app.py (Gradio UI)
                                     ...
                                     - uncolored asset-type composition
                                       chart + full holdings table
```
...
| `src/visualization.py` | Turns exposure/correlation records and raw holdings into chart-ready `pandas.DataFrame`s (concentration/correlation bar charts, asset-type composition, a full holdings table) and defines the fixed status/severity color palettes used everywhere in the UI. |
...
| `data/*.json` | Five sample portfolios covering LOW, two distinct MEDIUM profiles (WATCH vs. FLAGGED correlation), HIGH, and CRITICAL severity, used by the sample picker in both front ends. |
| `data/*.csv` | Three downloadable CSV samples (starter template, diversified/LOW, concentrated/CRITICAL) for exercising the CSV upload path with known-good data. |
...
- **Color is reserved for risk, not composition.** The asset-type allocation chart is deliberately uncolored — there's no configured limit on asset type, so coloring it would imply a risk judgment that isn't actually being made. Composition and risk are visually distinct for this reason.
